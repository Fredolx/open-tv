...
pub const INCREASE_FONT_SIZE: &str = "increaseFontSize";
...
pub fn get_settings() -> Result<Settings> {
    ...
    let settings = Settings {
        ...
        increase_font_size: map.get(INCREASE_FONT_SIZE).and_then(|s| s.parse().ok()),
    };
    Ok(settings)
}
...
pub fn update_settings(settings: Settings) -> Result<()> {
    ...
    if let Some(increase_font_size) = settings.increase_font_size {
        map.insert(INCREASE_FONT_SIZE.to_string(), increase_font_size.to_string());
    }
