...
pub const DEFAULT_SUBTITLE_LANGUAGE: &str = "defaultSubtitleLanguage";
...
pub fn get_settings() -> Result<Settings> {
    ...
    let settings = Settings {
        ...
        default_subtitle_language: map.get(DEFAULT_SUBTITLE_LANGUAGE).map(|s| s.to_string()),
    };
    Ok(settings)
}
...
pub fn update_settings(settings: Settings) -> Result<()> {
    ...
    if let Some(lang) = settings.default_subtitle_language {
        map.insert(DEFAULT_SUBTITLE_LANGUAGE.to_string(), lang);
    }
